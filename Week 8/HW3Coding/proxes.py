...
def prox_csimplex(z, k):
	...
	min,max,sum = np.minimum, np.maximum, np.sum
	def DualDerivative(y): return sum(max(0, min(1, z - y))) - k
	def XStar(z, y): return max(0, min(1, z - y))
	YStar = bisect(DualDerivative, -sum(np.abs(z)), sum(np.abs(z)))
	return XStar(z, YStar)
